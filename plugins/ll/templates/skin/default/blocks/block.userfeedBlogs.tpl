                ...
                {foreach from=$aBlogs item=oBlog}
                    ...
                    <li>
                        ...
                        {if $oBlog->getType()=='close'}
                            <i title="{$aLang.blog_closed}" class="{if $oConfig->get('view.skin')=='synio'}icon-synio-topic-private{else}icon-lock{/if}"{if $oConfig->get('view.skin')=='synio'} style="margin-top: -5px"{/if}></i>
                        {/if}
                    </li>
                {/foreach}
            </ul>
