...
<form action="{router page='feedback'}" method="POST" id="feedback-form" class="feedback-form">
	<div class="wrapper-content">
		...
		<dl class="form-item">
			...
			<dd>
				{if $oConfig->GetValue('plugin.feedback.field.title') == 1}
					<input type="text" name="title" id="feedback-title" value="{$_aRequest.title}" class="input-text input-width-300 js-ajax-validate" />
				{else}
					<select name="title" id="feedback-title" class="input-width-300 js-ajax-validate">
						<option value="">&nbsp;</option>
		...
		<dl class="form-item">
			...
			<dd>
				<textarea name="text" id="feedback-text" rows="20" class="mce-editor input-width-300 js-ajax-validate">{$_aRequest.text}</textarea>
		...
		<dl class="form-item">
			...
			<dd>
				<img src="{cfg name='path.root.engine_lib'}/external/kcaptcha/index.php?{$_sPhpSessionName}={$_sPhpSessionId}" 
					 ...
					 class="captcha-image" title="{$aLang.plugin.feedback.refresh_captcha}" onMouseOver="this.style.cursor='pointer'"/>
				<input type="text" name="captcha" id="feedback-captcha" value="" maxlength="5" class="input-text input-width-100 js-ajax-validate" />
